...
def dataEntrega():
    while True:
        ...
        if data <= data_atual:
            dias_restantes = (data_vencimento - data_atual).days
            return data.strftime("%d/%m/%Y"), data_vencimento.strftime("%d/%m/%Y"), dias_restantes
        else:
...
def laco_Epi():
    ...
    while True:
        try:
            ...
            # Data
            data_entrega, data_vencimento, dias_restantes = dataEntrega()
            global separador
            lista.append((separador, nEpi, tamanhoEpi, qtd_, data_entrega, data_vencimento, dias_restantes))
    ...
    while True:
        try:
            ...
            if escolhido == "1" or escolhido == "novo registro":
                ...
                for item in lacoEpi:
                    separador, nomeEpi, tamanhoEpi, qtd_, data_entrega, data_vencimento, dias_restantes = item
                    print(f"\n{separador} Epi")
...
# Função para centralizar informações nas colunas, exceto na coluna "Nome do Funcionário"
def centralizar_informacoes(file_path):
    wb = load_workbook(file_path)
    ws = wb.active

    for row in ws.iter_rows(min_row=2):  # Pula o cabeçalho
        for cell in row:
            if cell.column_letter != 'B':  # Assume que a coluna B é "Nome do Funcionário"
...
def salvar_dados_excel(nomePosto, nomeFuncio, lacoEpi):
    ...
    if file_exists:
        ...
        df = pd.DataFrame(
            columns=["Funcionario", "Posto", "Epi", "Tamanho", "Quantidade", "Data de Entrega",
                     "Data de Vencimento", "Dias Restantes"])
    ...
    centralizar_informacoes('cadastro_epi.xlsx')
